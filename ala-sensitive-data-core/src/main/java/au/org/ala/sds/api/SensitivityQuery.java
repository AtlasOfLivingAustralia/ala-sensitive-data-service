package au.org.ala.sds.api;

import com.fasterxml.jackson.annotation.JsonInclude;
import com.fasterxml.jackson.annotation.JsonProperty;
import com.fasterxml.jackson.databind.annotation.JsonDeserialize;
import io.swagger.annotations.ApiModel;
import io.swagger.annotations.ApiModelProperty;
import io.swagger.annotations.Extension;
import io.swagger.annotations.ExtensionProperty;
import lombok.Builder;
import lombok.EqualsAndHashCode;
import lombok.Value;
import java.util.Map;

@JsonInclude(JsonInclude.Include.NON_EMPTY)
@JsonDeserialize(builder = SensitivityQuery.SensitivityQueryBuilder.class)
@Value
@Builder
@EqualsAndHashCode
@ApiModel(
    description = "The basic information needed to process an occurrence record, including location, etc. " +
        "Information from the occurrence record is supplied as a set of properties, keyed by field name " +
        "-- the non-namespace part of a Darwin Core URI. " +
        "The result from the query may choose to process the values to meet the sensitivity rules."
)
public class SensitivityQuery {
    @ApiModelProperty(
        value = "The scientific name",
        example = "Eucalyptus rossii",
        extensions = {
            @Extension(
                name = "x-reference",
                properties = {
                    @ExtensionProperty(
                        name = "uri",
                        value = "http://rs.tdwg.org/dwc/terms/scientificName"
                    )
                }
            )
        }
    )
    @JsonProperty
    private String scientificName;
    @ApiModelProperty(
        value = "The taxon identifier",
        example = "https://id.biodiversity.org.au/node/apni/2900822",
        extensions = {
            @Extension(
                name = "x-reference" ,
                properties = {
                    @ExtensionProperty(
                        name = "uri" ,
                        value = "http://rs.tdwg.org/dwc/terms/taxonID"
                    )
                }
            )
        }
    )
    @JsonProperty
    private String taxonId;
    @ApiModelProperty(
        value = "The occurrence record properties"
    )
    @JsonProperty
    private Map<String, String> properties;

}
